...
public abstract class MontySolrTestCaseJ4 extends LuceneTestCase {
  ...
  public static Logger log = LoggerFactory.getLogger(MontySolrTestCaseJ4.class);
   ...
   */
	...
	private static final String SOLR_HOME = getSolrHome();
	private static final String SOURCE_HOME = MONTYSOLR_HOME + "/src/test-files";
	...
	/**
	 * MontySolr depends on the properties file for unittesting, as it was too much
	 * hassle to load interpolated strings from the build.properties file (without
	 * adding more jars) and the discovery of files using file structure crawling
	 * is ugly
	 * 
	 * @return
	 */
	private static Properties loadProperties() {
	    ...
	    try {
	        prop.load(new FileInputStream(new File(base + "/build/build.properties")));
	    } catch (IOException e) {
	    	fail("Your montysolr installation does not have " + base + "build/build.properties file!");
	    }
	    return prop;
	}
	...
	static String getSolrHome() {
	    ...
	    if (solr_home == null || !(new File(solr_home).exists())) {
	    	fail("Your montysolr build.properties file has incorrect value for solr.home: " + solr_home);
	    }
	    ...
	    fail("Cannot determine the folder with solr installation");
	    return null;
	}
