define([
  ...
  var ToggleList = React.createClass({
    ...
    render: function () {
      ...
      var data = (this.props.currentLevel === 1 ) ? this.props.reduxState : this.props.reduxState.facets[this.props.id],
      ...
      if (data.pagination.state === "loading") {
        ...
        stateMessage = <span><i className="icon-danger"></i> request failed</span>
      }
      ...
      if (!facets.length) {
        ...
        var list = facets.slice(0, visible)
            .map(function (c) {

              var props = {
                isChecked: this.props.reduxState.state.selected.indexOf(c.value) > -1 ? true : false,
                name: c.name,
                count: c.count,
                hierarchical: this.props.reduxState.config.hierMaxLevels > this.props.currentLevel ? true : false,
                value: c.value,
                id: c.value,
                selectFacet: this.props.selectFacet,
                unselectFacet: this.props.unselectFacet
              ...
              if (props.hierarchical) {
                _.extend(props, _.pick(this.props, ["showMoreFacets", "resetVisibleFacets", "toggleFacet", "reduxState", "currentLevel"]));
              }
      ...
      var parentClass = this.props.currentLevel >1 ? "facet__list-container facet__list-container--child" : "facet__list-container";
      ...
      if ( open ){
        facetList = <div className={ parentClass }>
              ...
              <button className={lessButtonClasses}
                      onClick={ _.partial(this.props.resetVisibleFacets, this.props.id) }>
                less
              </button>
      ...
      return <div>
        <div className="facet__toggle">
          <i className={open ? "facet__icon facet__icon--open" : "facet__icon facet__icon--closed" }
