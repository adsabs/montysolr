...
public class BatchHandler extends RequestHandlerBase {
    ...
    private final List<String> workerMessage;
    private long sleepTime;
    private final Map<String, BatchProvider> providers;
    ...
    public void init(NamedList args) {
        ...
        if (defs.get("asynchronous") != null) {
            asynchronous = (Boolean) defs.get("asynchronous");
        }
    ...
    private void checkHomeDir() {
        ...
        if (defaultWorkdir != null) {
            ...
            if (wdir.isAbsolute() && wdir.canWrite()) {
                ...
                if (!wdir.isAbsolute()) {
                    wdir = new File(startDir + "/" + wdir);
                }
                wdir.mkdir();
                tmpDir = wdir;
            } else {
                throw new RuntimeException("The folder is not readable: " + wdir);
            }
        } else {
