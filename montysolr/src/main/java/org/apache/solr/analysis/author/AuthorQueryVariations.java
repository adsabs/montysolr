    ...
    protected static Set<String> generateNameVariations(Map<String, String> parsedAuthor, Set<String> variations) {
        ...
        if (first != null) {
            if (middle != null) {
                if (first.length() > 1) {
                    if (middle.length() > 1) {
                        variations.add(last + ", " + first + " " + middle.charAt(0) + "\\b.*");
                    } else {
                        variations.add(last + ", " + first + " " + middle + ".*");
                        variations.add(last + ", " + first.charAt(0) + " " + middle + ".*");
                    }
                } else {
                    if (middle.length() > 1) {
                        variations.add(last + ", " + first + "\\w* " + middle.charAt(0) + "\\b.*");
                    }
                }
            } else {
                if (first.length() > 1) {
                    variations.add(last + ", " + first.charAt(0) + "\\b.*");
                }
            }
        }
            ...
            if (middle != null) {
                if (first.length() > 1) {
                    ...
                    if (middle.length() > 1) {
                        ...
                        variations.add(last + ", " + first.charAt(0) + " " + middle.charAt(0) + "\\b.*");
