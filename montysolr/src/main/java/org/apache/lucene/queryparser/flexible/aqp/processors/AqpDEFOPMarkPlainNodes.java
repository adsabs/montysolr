...
public class AqpDEFOPMarkPlainNodes extends AqpQProcessor {
    ...
    private final List<String> firstChildAllowedModifiers;
    private final List<String> firstChildAllowedFields;

    public AqpDEFOPMarkPlainNodes() {
        modifyTree = false;
        firstChildAllowedModifiers = Arrays.asList("+", "-");
        firstChildAllowedFields = Collections.singletonList("");
    }
        ...
        if (forMarking.size() > 1) {
            ...
            for (int i = 0; i < forMarking.size(); i++) {
                ...
                if (modifyTree) {
                    ...
                    if (startPos == -1)
                        startPos = terminal.getInputTokenStart();
                    endPos = terminal.getInputTokenEnd();
                }
            }
        }
    ...
    private boolean isBareNode(QueryNode node, boolean isPotentiallyFirst) {
        ...
        if (sb.toString().equals("/MODIFIER/TMODIFIER/FIELD/QNORMAL") ||
            ...
            if (vals.size() == 0) {
                ...
                // or certain fields
                return isPotentiallyFirst &&
                        (firstChildAllowedModifiers.contains(vals.get(0)) || // we allow modifiers for the first child
                                firstChildAllowedFields.contains(vals.get(0)));
            } else if (vals.size() == 2) {
                // in certain fields
                return isPotentiallyFirst &&
                        (firstChildAllowedModifiers.contains(vals.get(0)) && // we allow modifiers for the first child
                                firstChildAllowedFields.contains(vals.get(1)));
            } else {
                return false;
            }
        }
