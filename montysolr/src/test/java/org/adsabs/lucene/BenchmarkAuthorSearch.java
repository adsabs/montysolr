...
public class BenchmarkAuthorSearch extends LuceneTestCase {
    ...
    private final long maxTime = 60 * 1000; // max time benchmark is allowed to run
    private final ArrayList<ArrayList<Object>> timerStack = new ArrayList<ArrayList<Object>>();

    private final String[] names = {"a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "x",
    ...
    public void tearDown() throws Exception {
        ...
        assertEquals(0, timerStack.size());
    }
    ...
    static final class SimplePayloadFilter extends TokenFilter {
        ...
        public boolean incrementToken() throws IOException {
            if (input.incrementToken()) {
                payloadAttr.setPayload(new BytesRef((Integer.toString(pos)).getBytes(StandardCharsets.UTF_8)));  //save the position 0 = surname,1,2,3,4....
                pos++;
                return true;
            } else {
                return false;
    ...
    private List<TestCase> getIndexData(int[] randomIds) throws IOException {
        ...
        for (int i = 0; i < randomIds.length; i++) {
            ...
            String original = doc.get("original");
    ...
    private Query getSpanQuery(String[] parts, int howMany, boolean truncate) throws UnsupportedEncodingException {
        ...
        for (int i = 0; i < howMany; i++) {
            if (truncate) {
                SpanMultiTermQueryWrapper<WildcardQuery> q = new SpanMultiTermQueryWrapper<WildcardQuery>(new WildcardQuery(new Term("vectrfield", parts[i + 1].charAt(0) + "*")));
                clauses[i + 1] = q;
            } else {
    ...
    private Query getPayloadQuery(String[] parts, int howMany, boolean truncate) throws IOException {
        ...
        BytesRef pay = new BytesRef((Integer.toString(0)).getBytes(StandardCharsets.UTF_8));
        payloads.add(pay);
        ...
        for (int i = 0; i < howMany; i++) {
            ...
            payloads.add(new BytesRef((Integer.toString(i + 1)).getBytes(StandardCharsets.UTF_8)));
        }
    ...
    private Query getBooleanQuery(String[] parts, int howMany, boolean truncate) {
        ...
        for (int i = 1; i < howMany + 1; i++) {
            if (truncate) {
                bq.add(new WildcardQuery(new Term("n" + i, parts[i].charAt(0) + "*")), BooleanClause.Occur.MUST);
            } else {
