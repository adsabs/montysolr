define(['jquery',
  ...
  describe("Citation Widget (UI Widget)", function(){
    ...
    it("has a loadBibcodeInfo function that takes a bibcode, requests  citations(bibcode:data), and returns a promise", function(){
      ...
      expect(sentRequest.get("query").get("q")[0]).to.equal("citations(bibcode:sampleBib1)")

      //sort should be pubdate desc
      expect(sentRequest.get("query").get("sort")[0]).to.equal("pubdate desc")
