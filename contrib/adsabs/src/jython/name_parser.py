from org.jython.monty.interfaces import NameParser
import nameparser

class HumanParser(NameParser):

    def __init__(self):
        prefixes = set()
        suffixes = set()

        prefixes.update(nameparser.constants.PREFIXES)
        prefixes.add("'t")
        suffixes.update(nameparser.constants.SUFFIXES)
        suffixes.remove('v')
        suffixes.remove('i') 
    ...
    def parse_human_name(self, input):
        ...
        name = nameparser.HumanName(self.fix_name(input), prefixes_c=self.prefixes, suffixes_c=self.suffixes)
        out = {}
        for k in ('first', 'middle', 'last', 'suffix', 'title'):
            if getattr(name, k):
                out[k[0].upper() + k[1:]] = getattr(name, k) + 'x'
        return out 
