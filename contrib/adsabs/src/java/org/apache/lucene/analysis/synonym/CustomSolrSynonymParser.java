 ...
 * @lucene.experimental
 */
public class CustomSolrSynonymParser extends SynonymMap.Builder {
  private final boolean expand;
  private final Analyzer analyzer;
  private boolean inclOrig;
  private boolean sanitizeOutput;
  
  public CustomSolrSynonymParser(boolean dedup, boolean expand, Analyzer analyzer) {
    super(dedup);
    this.expand = expand;
  ...
  public void setInclOrig(boolean v) {
    inclOrig = v;
  }
  public void setSanitizeOutput(boolean v) {
    sanitizeOutput = v;
  }
    ...
    while ((line = in.readLine()) != null) {
      ...
      for (int i = 0; i < inputs.length; i++) {
        for (int j = 0; j < outputs.length; j++) {
          add(inputs[i], sanitizeOutput ? sanitize(outputs[j]) : outputs[j], inclOrig);
        }
      }
    }
  }
  ...
  private CharsRef sanitize(CharsRef charsRef) {
