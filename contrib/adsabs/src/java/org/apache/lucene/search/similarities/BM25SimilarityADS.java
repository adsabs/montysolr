 ...
 */
public class BM25SimilarityADS extends Similarity {
   ...
   */
  public BM25SimilarityADS(float k1, float b) {
  ...
  /** BM25 uses L factor defined as |d|/avgDocLength ; we will try to 
   * ignore boost (which is index boost only) in the future we can experiment
   * with boost */
  ...
  protected float decodeNormValue(byte b) {
    return SmallFloat.byte315ToFloat(b);
  }
  ...
  public final SimWeight computeWeight(CollectionStatistics collectionStats, TermStatistics... termStats) {
    ...
    return new BM25StatsADS(collectionStats.field(), idf, avgdl, cache);
  }

  @Override
  public final SimScorer simScorer(SimWeight stats, LeafReaderContext context) throws IOException {
    BM25StatsADS bm25stats = (BM25StatsADS) stats;
    return new BM25DocScorer(bm25stats, context.reader().getNormValues(bm25stats.field));
  }
  
  private class BM25DocScorer extends SimScorer {
    private final BM25StatsADS stats;
    private final float weightValue; // boost * idf * (k1 + 1)
    private final NumericDocValues norms;
    private final float[] cache;
    
    BM25DocScorer(BM25StatsADS stats, NumericDocValues norms) throws IOException {
      this.stats = stats;
  ...
  private static class BM25StatsADS extends SimWeight {
    /** BM25's idf */
